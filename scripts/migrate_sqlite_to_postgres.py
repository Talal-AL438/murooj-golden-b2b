...
CREATE TABLE IF NOT EXISTS offers (
    ...
    manual_language TEXT DEFAULT 'ar',
    active INTEGER DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS offer_targets (
    ...
    agency_id BIGINT NOT NULL REFERENCES agencies(id),
    PRIMARY KEY (offer_id, agency_id)
);
...
CREATE TABLE IF NOT EXISTS pending_admin_devices (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES users(id),
...
def reset_sequence(cur, table):
    ...
    cur.execute(
        sql.SQL(
            "SELECT setval(pg_get_serial_sequence(%s, 'id'), COALESCE((SELECT MAX(id) FROM {}), 1), COALESCE((SELECT MAX(id) FROM {}), 0) > 0)"
...
def main():
    ...
    with closing(sqlite3.connect(SOURCE_SQLITE)) as src:
        ...
        with closing(psycopg2.connect(DATABASE_URL)) as pg:
            pg.autocommit = False
            try:
                with pg.cursor() as cur:
                    ...
                    for table in COPY_ORDER:
                        if table != "settings" and table != "offer_targets" and table != "staff_permissions":
