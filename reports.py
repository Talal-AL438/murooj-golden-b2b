...
def register_reports(app, admin_required, db, current_user):
    ...
    def portal_extensions_and_staff_access():
        ...
        if request.path=='/request' and request.method=='POST':
            ...
            cur=c.execute("INSERT INTO requests(agency_id,hotel_id,any_hotel,city,checkin,checkout,rooms,persons,nationality,meal,notes,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(agency['id'],hotel_id,1 if any_hotel else 0,city,checkin,checkout,rooms,persons,nationality,meal,notes,'sent',datetime.utcnow().isoformat()));rid=cur.lastrowid;notification_text={
                'ar':('تحديث الطلب',f'تم استلام الطلب #{rid} بنجاح.'),
                'en':('Request update',f'Request #{rid} has been received.'),
                'id':('Pembaruan permintaan',f'Permintaan #{rid} telah diterima.'),
                'ms':('Kemas kini permintaan',f'Permintaan #{rid} telah diterima.')}
            ntitle,nbody=notification_text.get(lang,notification_text['ar']);c.execute("INSERT INTO notifications(user_id,type,ref_id,title,body,link,created_at) VALUES(?,?,?,?,?,?,?)",(u['id'],'request',rid,ntitle,nbody,'/account',datetime.utcnow().isoformat()));audit(c,u['id'],'request_submit',f"request={rid}, agency={agency['id']}");c.commit();c.close();flash({'ar':'تم إرسال طلبك إلى مروج الذهبية بنجاح. سيتواصل معك فريق الحجوزات عبر WhatsApp لتأكيد التوفر والسعر.','en':'Your request was sent to Murooj Golden successfully. Our reservations team will contact you on WhatsApp to confirm availability and price.','id':'Permintaan Anda berhasil dikirim ke Murooj Golden. Tim reservasi akan menghubungi Anda melalui WhatsApp untuk mengonfirmasi ketersediaan dan harga.','ms':'Permintaan anda berjaya dihantar kepada Murooj Golden. Pasukan tempahan akan menghubungi anda melalui WhatsApp untuk mengesahkan ketersediaan dan harga.'}.get(lang,'Request sent.'));return redirect(url_for('account'))
