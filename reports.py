...
def register_reports(app, admin_required, db, current_user):
    ...
    def ensure_permissions_table(c):
        c.execute("""CREATE TABLE IF NOT EXISTS staff_permissions (
            user_id INTEGER PRIMARY KEY,
            reports INTEGER DEFAULT 0,
            agencies INTEGER DEFAULT 0,
    ...
    def get_permissions(user_id):
        c=db();ensure_permissions_table(c)
        row=c.execute("SELECT * FROM staff_permissions WHERE user_id=?",(user_id,)).fetchone();c.close()
        if not row:return {k:False for k in permission_keys}
    ...
    def portal_extensions_and_staff_access():
        ...
        if needed and not get_permissions(u['id']).get(needed,False):
            flash('ليس لديك صلاحية للوصول إلى هذا القسم.' if (u['language'] or 'ar')=='ar' else 'You do not have permission to access this section.')
            return redirect(url_for('admin'))
        return None

    @app.context_processor
    def extended_portal_context():
        u=current_user(); country=''; staff_permissions={k:True for k in permission_keys}
        ...
        elif u and u['role']=='staff':
            staff_permissions=get_permissions(u['id'])
        return {'agency_country':country,'current_date':datetime.utcnow().date().isoformat(),'staff_permissions':staff_permissions}
    ...
    def update_staff_permissions(uid):
        ...
        c=db();ensure_permissions_table(c)
        employee=c.execute("SELECT id,role,email FROM users WHERE id=?",(uid,)).fetchone()
        ...
        c.execute("""INSERT INTO staff_permissions(user_id,reports,agencies,hotels,offers,updated_at)
                     VALUES(?,?,?,?,?,?)
                     ON CONFLICT(user_id) DO UPDATE SET reports=excluded.reports,agencies=excluded.agencies,hotels=excluded.hotels,offers=excluded.offers,updated_at=excluded.updated_at""",
                  (uid,vals[0],vals[1],vals[2],vals[3],datetime.utcnow().isoformat()))
        c.commit();c.close();flash('تم تحديث صلاحيات الموظف.')
    ...
    def toggle_staff(uid):
        ...
        c.execute("UPDATE users SET active=? WHERE id=?",(0 if row['active'] else 1,uid));c.commit();c.close();flash('تم تحديث حالة الموظف.')
        return redirect(url_for('admin_employees'))

    def filters():
        return {'country':request.args.get('country','').strip(),'hotel':request.args.get('hotel','').strip(),'status':request.args.get('status','').strip(),'from':request.args.get('from','').strip(),'to':request.args.get('to','').strip()}

    def query_rows(c,f):
        sql="""SELECT r.*,a.agency_name,a.country,a.contact_name,a.whatsapp,u.email,h.name_ar,h.name_en
                 FROM requests r JOIN agencies a ON a.id=r.agency_id JOIN users u ON u.id=a.user_id
                 LEFT JOIN hotels h ON h.id=r.hotel_id WHERE 1=1""";params=[]
        if f['country']:sql+=' AND a.country LIKE ?';params.append('%'+f['country']+'%')
        if f['hotel']:
            sql+=" AND (h.name_ar LIKE ? OR h.name_en LIKE ? OR (r.any_hotel=1 AND ? LIKE '%any%'))";params += ['%'+f['hotel']+'%','%'+f['hotel']+'%',f['hotel'].lower()]
        ...
        sql+=' ORDER BY r.id DESC';return c.execute(sql,params).fetchall()
    ...
    def admin_reports_excel():
        ...
        for r in rows:
            hotel='Any Available Hotel' if r['any_hotel'] else (r['name_en'] or r['name_ar'] or '');ws.append([r['id'],r['agency_name'],r['country'],r['contact_name'],r['whatsapp'],r['email'],hotel,r['city'],r['checkin'],r['checkout'],r['rooms'],r['persons'],r['nationality'],r['meal'],r['status'],r['created_at']])
        ws.freeze_panes='A2';ws.auto_filter.ref=ws.dimensions
        for i in range(1,len(headers)+1):
            max_len=max(len(str(ws.cell(row=j,column=i).value or '')) for j in range(1,min(ws.max_row,500)+1));ws.column_dimensions[get_column_letter(i)].width=min(max(max_len+2,12),32)
