...
LANGS = {
    "ar": {
        ...
        "hero":"شريكك الموثوق لحجوزات الفنادق في مكة المكرمة والمدينة المنورة","sub":"بوابة B2B مخصصة لوكالات السفر والحج والعمرة.",
    ...
    "en": {
        ...
        "hero":"Your Trusted Hotel Partner in Makkah and Madinah","sub":"A B2B portal for travel, Hajj and Umrah agencies.",
        ...
        "category":"Category","last_request":"Last Request","request_count":"Requests","registration_date":"Registered","no_active_offers":"No active offers at the moment"
    },
    "id": {
        ...
        "hero":"Mitra Hotel Tepercaya Anda di Makkah dan Madinah","sub":"Portal B2B untuk agen perjalanan, Haji dan Umrah.",
        ...
        "category":"Kategori","last_request":"Permintaan Terakhir","request_count":"Jumlah Permintaan","registration_date":"Tanggal Daftar","no_active_offers":"Saat ini tidak ada penawaran aktif"
    },
    "ms": {
        ...
        "hero":"Rakan Hotel Dipercayai Anda di Makkah dan Madinah","sub":"Portal B2B untuk agensi pelancongan, Haji dan Umrah.",
        ...
        "category":"Kategori","last_request":"Permintaan Terakhir","request_count":"Jumlah Permintaan","registration_date":"Tarikh Daftar","no_active_offers":"Tiada tawaran aktif buat masa ini"
    }
}
...
def init_db():
    ...
    """)
    for k,v in {"company_name":"مروج الذهبية للاستثمار","brand":"MUROOJ GOLDEN","whatsapp":"966550558014","email":"talal_alaqely@icloud.com","announcement":"","announcement_active":"0"}.items():
        conn.execute("INSERT OR IGNORE INTO settings(key,value) VALUES (?,?)",(k,v))
    if conn.execute("SELECT COUNT(*) c FROM hotels").fetchone()["c"]==0:
        now=datetime.utcnow().isoformat()
        conn.execute("INSERT INTO hotels(name_ar,name_en,city,map_url,services,meals,active,sort_order,created_at) VALUES(?,?,?,?,?,?,?,?,?)",("فندق ندى أجياد","NADA AJYAD HOTEL","Makkah","https://maps.app.goo.gl/PzgRgz23LRDex1Wq7?g_st=iw","Wi‑Fi|استقبال 24 ساعة|مصاعد|تكييف|مطعم|تنظيف الغرف|ثلاجة|تلفزيون","RO|F.B Indo|F.B Malaysian",1,1,now))
        conn.execute("INSERT INTO hotels(name_ar,name_en,city,map_url,services,meals,active,sort_order,created_at) VALUES(?,?,?,?,?,?,?,?,?)",("فندق سواعد الخير","SAWAEED AL KHAIR HOTEL","Makkah","https://maps.app.goo.gl/GcA7zZYftYcuYx9N6?g_st=iw","Wi‑Fi|استقبال 24 ساعة|مصاعد|تكييف|مطعم|تنظيف الغرف|ثلاجة|تلفزيون","RO|F.B Indo|F.B Malaysian",1,2,now))
...
def log(action,details=""):
    u=current_user(); conn=db(); conn.execute("INSERT INTO audit(user_id,action,details,created_at) VALUES(?,?,?,?)",(u["id"] if u else None,action,details,datetime.utcnow().isoformat())); conn.commit(); conn.close()
...
@app.route("/")
def home():
    conn=db(); hotels=conn.execute("SELECT * FROM hotels WHERE active=1 ORDER BY sort_order,id").fetchall(); today=datetime.utcnow().date().isoformat()
    offers=conn.execute("""SELECT o.*,h.name_ar,h.name_en FROM offers o LEFT JOIN hotels h ON h.id=o.hotel_id WHERE o.active=1 AND (o.end_date='' OR o.end_date IS NULL OR o.end_date>=?) ORDER BY o.pinned DESC,o.sort_order,o.id DESC LIMIT 6""",(today,)).fetchall()
...
def register():
    if request.method=="POST":
        if not request.form.get("privacy") or not request.form.get("marketing"):
            flash("Both privacy and WhatsApp marketing consent are required."); return redirect(url_for("register"))
        email=request.form["email"].strip().lower(); conn=db()
        if conn.execute("SELECT 1 FROM users WHERE email=?",(email,)).fetchone(): conn.close(); flash("Email already registered."); return redirect(url_for("register"))
        now=datetime.utcnow().isoformat(); cur=conn.execute("INSERT INTO users(email,password_hash,role,name,mobile,language,created_at) VALUES(?,?,?,?,?,?,?)",(email,generate_password_hash(request.form["password"]),"agency",request.form["contact_name"],request.form["whatsapp"],request.form.get("language","ar"),now)); uid=cur.lastrowid
        conn.execute("INSERT INTO agencies(user_id,agency_name,country,contact_name,whatsapp,marketing_consent,created_at) VALUES(?,?,?,?,?,?,?)",(uid,request.form["agency_name"],request.form["country"],request.form["contact_name"],request.form["whatsapp"],1,now)); conn.commit(); conn.close(); session["user_id"]=uid; session["lang"]=request.form.get("language","ar"); return redirect(url_for("account"))
...
def setup_admin():
    conn=db(); exists=conn.execute("SELECT 1 FROM users WHERE role='super_admin'").fetchone()
    if exists: conn.close(); return redirect(url_for("login"))
    if request.method=="POST":
        email=request.form["email"].strip().lower()
        if email!="talal_alaqely@icloud.com": conn.close(); flash("Use the approved temporary admin email."); return redirect(url_for("setup_admin"))
        conn.execute("INSERT INTO users(email,password_hash,role,name,language,created_at) VALUES(?,?,?,?,?,?)",(email,generate_password_hash(request.form["password"]),"super_admin",request.form.get("name","Super Admin"),"ar",datetime.utcnow().isoformat())); conn.commit(); conn.close(); flash("Super Admin account created. You can now log in."); return redirect(url_for("login"))
...
def new_request():
    ...
    conn=db(); agency=conn.execute("SELECT * FROM agencies WHERE user_id=?",(u["id"],)).fetchone(); hotels=conn.execute("SELECT * FROM hotels WHERE active=1 ORDER BY sort_order,id").fetchall()
    if request.method=="POST":
        hotel_val=request.form.get("hotel_id",""); any_hotel=1 if hotel_val=="any" else 0; hotel_id=None if any_hotel else int(hotel_val); rooms=int(request.form["rooms"]); persons=int(request.form["persons"])
        if rooms<1 or persons<1 or request.form["checkout"]<request.form["checkin"]: conn.close(); return "invalid request",400
        conn.execute("INSERT INTO requests(agency_id,hotel_id,any_hotel,city,checkin,checkout,rooms,persons,nationality,meal,notes,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(agency["id"],hotel_id,any_hotel,request.form["city"],request.form["checkin"],request.form["checkout"],rooms,persons,request.form["nationality"],request.form["meal"],request.form.get("notes",""),"sent",datetime.utcnow().isoformat())); conn.commit(); flash(t()["success_request"]); conn.close(); return redirect(url_for("account"))
    conn.close(); return render_template("request.html",hotels=hotels,user=u)
...
def account():
    u=current_user()
    if u["role"]!="agency": return redirect(url_for("admin"))
    conn=db(); agency=conn.execute("SELECT * FROM agencies WHERE user_id=?",(u["id"],)).fetchone()
    if request.method=="POST":
        ...
        if action=="profile":
            ...
            if cn and country and wa: conn.execute("UPDATE agencies SET contact_name=?,country=?,whatsapp=? WHERE id=?",(cn,country,wa,agency["id"])); conn.execute("UPDATE users SET name=?,mobile=? WHERE id=?",(cn,wa,u["id"])); conn.commit(); flash(m["profile"])
            ...
            else: conn.execute("UPDATE users SET password_hash=? WHERE id=?",(generate_password_hash(np),u["id"])); conn.commit(); flash(m["password"])
        conn.close(); return redirect(url_for("account"))
    reqs=conn.execute("SELECT r.*,h.name_ar,h.name_en FROM requests r LEFT JOIN hotels h ON h.id=r.hotel_id WHERE r.agency_id=? ORDER BY r.id DESC",(agency["id"],)).fetchall(); conn.close(); return render_template("account.html",user=u,agency=agency,reqs=reqs)

@app.route("/admin")
@admin_required
def admin():
    conn=db(); stats={"agencies":conn.execute("SELECT COUNT(*) c FROM agencies").fetchone()["c"],"new":conn.execute("SELECT COUNT(*) c FROM requests WHERE status='sent'").fetchone()["c"],"requests":conn.execute("SELECT COUNT(*) c FROM requests").fetchone()["c"],"hotels":conn.execute("SELECT COUNT(*) c FROM hotels WHERE active=1").fetchone()["c"]}; reqs=conn.execute("SELECT r.*,a.agency_name,a.country,a.whatsapp,h.name_ar,h.name_en FROM requests r JOIN agencies a ON a.id=r.agency_id LEFT JOIN hotels h ON h.id=r.hotel_id ORDER BY r.id DESC LIMIT 50").fetchall(); conn.close(); return render_template("admin.html",user=current_user(),stats=stats,reqs=reqs)
...
def update_request_status(rid):
    ...
    conn=db(); conn.execute("UPDATE requests SET status=? WHERE id=?",(status,rid)); conn.commit(); conn.close(); log("request_status",f"request={rid}, status={status}"); return redirect(url_for("admin"))
...
def admin_hotels():
    conn=db()
    if request.method=="POST":
        action=request.form.get("action","add"); data=(request.form["name_ar"].strip(),request.form["name_en"].strip(),request.form["city"],request.form.get("map_url","").strip(),request.form.get("services","").strip(),request.form.get("meals","").strip(),int(request.form.get("sort_order",0)))
        if action=="edit":
            hid=int(request.form["hid"]); conn.execute("UPDATE hotels SET name_ar=?,name_en=?,city=?,map_url=?,services=?,meals=?,sort_order=? WHERE id=?",data+(hid,)); conn.commit(); conn.close(); log("hotel_edit",f"hotel={hid}"); return redirect(url_for("admin_hotels"))
        conn.execute("INSERT INTO hotels(name_ar,name_en,city,map_url,services,meals,active,sort_order,created_at) VALUES(?,?,?,?,?,?,1,?,?)",data+(datetime.utcnow().isoformat(),)); conn.commit(); conn.close(); log("hotel_add",data[1]); return redirect(url_for("admin_hotels"))
    hotels=conn.execute("SELECT * FROM hotels ORDER BY sort_order,id").fetchall(); conn.close(); return render_template("admin_hotels.html",user=current_user(),hotels=hotels)
...
def hotel_toggle(hid):
    ...
    if not row: conn.close(); return "hotel not found",404
    conn.execute("UPDATE hotels SET active=? WHERE id=?",(0 if row["active"] else 1,hid)); conn.commit(); conn.close(); log("hotel_toggle",f"hotel={hid}"); return redirect(url_for("admin_hotels"))
...
def admin_offers():
    conn=db()
    if request.method=="POST":
        conn.execute("INSERT INTO offers(title,hotel_id,start_date,end_date,meal,note,audience,language_mode,manual_language,active,pinned,sort_order,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(request.form["title"],request.form.get("hotel_id") or None,request.form.get("start_date",""),request.form.get("end_date",""),request.form.get("meal",""),request.form.get("note",""),request.form.get("audience","all"),request.form.get("language_mode","auto"),request.form.get("manual_language","ar"),1,1 if request.form.get("pinned") else 0,int(request.form.get("sort_order",0)),datetime.utcnow().isoformat())); conn.commit(); log("offer_add",request.form["title"])
    offers=conn.execute("SELECT o.*,h.name_en FROM offers o LEFT JOIN hotels h ON h.id=o.hotel_id ORDER BY o.pinned DESC,o.sort_order,o.id DESC").fetchall(); hotels=conn.execute("SELECT * FROM hotels WHERE active=1 ORDER BY sort_order,id").fetchall(); conn.close(); return render_template("admin_offers.html",user=current_user(),offers=offers,hotels=hotels)
...
def admin_agencies():
    ...
    if request.method=="POST":
        ...
        except ValueError: conn.close(); return "bad agency",400
        agency=conn.execute("SELECT * FROM agencies WHERE id=?",(aid,)).fetchone()
        if not agency: conn.close(); return "agency not found",404
        action=request.form.get("action","save")
        if action=="save":
            category=request.form.get("category","New")
            if category not in ("New","Active","VIP","Suspended"): conn.close(); return "bad category",400
            verified=1 if request.form.get("verified") else 0; notes=request.form.get("internal_notes","").strip(); conn.execute("UPDATE agencies SET category=?,verified=?,internal_notes=? WHERE id=?",(category,verified,notes,aid)); conn.execute("UPDATE users SET active=? WHERE id=?",(0 if category=="Suspended" else 1,agency["user_id"]))
        elif action=="toggle":
            user=conn.execute("SELECT active FROM users WHERE id=?",(agency["user_id"],)).fetchone(); new_active=0 if user and user["active"] else 1; conn.execute("UPDATE users SET active=? WHERE id=?",(new_active,agency["user_id"])); conn.execute("UPDATE agencies SET category=? WHERE id=?",("Suspended" if not new_active else ("Active" if agency["category"]=="Suspended" else agency["category"]),aid))
        else: conn.close(); return "bad action",400
        conn.commit(); conn.close(); log("agency_update",f"agency={aid}, action={action}"); return redirect(url_for("admin_agencies"))
    ...
    sql+=" ORDER BY a.id DESC"; agencies=conn.execute(sql,params).fetchall(); conn.close(); return render_template("admin_agencies.html",user=current_user(),agencies=agencies,q=q)
...
def admin_employees():
    ...
    if request.method=="POST":
        email=request.form["email"].strip().lower()
        if not conn.execute("SELECT 1 FROM users WHERE email=?",(email,)).fetchone(): conn.execute("INSERT INTO users(email,password_hash,role,name,job_title,mobile,language,active,created_at) VALUES(?,?,?,?,?,?,?,?,?)",(email,generate_password_hash(request.form["password"]),"staff",request.form["name"],request.form["job_title"],request.form["mobile"],"ar",1,datetime.utcnow().isoformat())); conn.commit(); log("employee_add",email)
    employees=conn.execute("SELECT * FROM users WHERE role IN ('staff','super_admin') ORDER BY id").fetchall(); conn.close(); return render_template("admin_employees.html",user=current_user(),employees=employees)
...
def admin_settings():
    ...
    if request.method=="POST":
        for key in ("company_name","brand","whatsapp","email","announcement","announcement_active"):
            if key in request.form: conn.execute("INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",(key,request.form[key]))
        conn.commit(); log("settings_update","general settings")
    settings={r["key"]:r["value"] for r in conn.execute("SELECT * FROM settings").fetchall()}; conn.close(); return render_template("admin_settings.html",user=current_user(),settings=settings)

@app.route("/admin/audit")
@super_required
def admin_audit():
    conn=db(); rows=conn.execute("SELECT a.*,u.email,u.name FROM audit a LEFT JOIN users u ON u.id=a.user_id ORDER BY a.id DESC LIMIT 300").fetchall(); conn.close(); return render_template("admin_audit.html",user=current_user(),rows=rows)
