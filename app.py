...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS hotels (id INTEGER PRIMARY KEY AUTOINCREMENT,name_ar TEXT NOT NULL,name_en TEXT NOT NULL,city TEXT NOT NULL,map_url TEXT,services TEXT,meals TEXT,stars INTEGER DEFAULT 3,active INTEGER DEFAULT 1,sort_order INTEGER DEFAULT 0,created_at TEXT NOT NULL);
...
def admin_hotels():
    ...
    if request.method=="POST":
        ...
        data=(request.form["name_ar"].strip(),request.form["name_en"].strip(),request.form["city"],request.form.get("map_url","").strip(),request.form.get("services","").strip(),request.form.get("meals","").strip(),stars,int(request.form.get("sort_order",0)))
        if action=="edit":
            hid=int(request.form["hid"]);c.execute("UPDATE hotels SET name_ar=?,name_en=?,city=?,map_url=?,services=?,meals=?,stars=?,sort_order=? WHERE id=?",data+(hid,));c.commit();c.close();log("hotel_edit",f"hotel={hid}");return redirect(url_for("admin_hotels"))
        c.execute("INSERT INTO hotels(name_ar,name_en,city,map_url,services,meals,stars,active,sort_order,created_at) VALUES(?,?,?,?,?,?,?,1,?,?)",data+(datetime.utcnow().isoformat(),));c.commit();c.close();log("hotel_add",data[1]);return redirect(url_for("admin_hotels"))
...
from marketing_extensions import register_marketing_extensions
register_marketing_extensions(app,db,current_user)
