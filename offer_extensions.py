...
def register_offer_extensions(app, db, current_user):
    ...
    def ensure_offer_schema():
        ...
        if request.path=='/register' and request.method=='POST':
            ...
            messages={'ar':{'bad':'يرجى إدخال بيانات الوكالة بشكل صحيح.','password':'يجب أن تكون كلمة المرور 8 أحرف على الأقل.','consent':'يجب الموافقة على الشروط واستقبال تحديثات WhatsApp.'},'en':{'bad':'Please enter valid agency details.','password':'Password must be at least 8 characters.','consent':'You must accept the terms and WhatsApp updates.'},'id':{'bad':'Masukkan data agen yang valid.','password':'Kata sandi minimal 8 karakter.','consent':'Anda harus menyetujui syarat dan pembaruan WhatsApp.'},'ms':{'bad':'Sila masukkan maklumat agensi yang sah.','password':'Kata laluan mestilah sekurang-kurangnya 8 aksara.','consent':'Anda mesti bersetuju dengan terma dan kemas kini WhatsApp.'}}
    ...
    def employee_error(key):
        u=current_user();lang=(u['language'] if u else 'ar') or 'ar';messages={'ar':{'bad':'تحقق من اسم الموظف والقسم والمسمى الوظيفي والجوال والبريد الإلكتروني.','password':'كلمة المرور المؤقتة يجب أن تكون 8 أحرف على الأقل.','email':'البريد الإلكتروني مستخدم بالفعل.','saved':'تم تحديث بيانات الموظف بنجاح.','added':'تمت إضافة الموظف بنجاح.'},'en':{'bad':'Check the employee name, department, job title, mobile, and email.','password':'Temporary password must be at least 8 characters.','email':'This email is already in use.','saved':'Employee details updated successfully.','added':'Employee added successfully.'},'id':{'bad':'Periksa nama, departemen, jabatan, ponsel, dan email pegawai.','password':'Kata sandi sementara minimal 8 karakter.','email':'Email ini sudah digunakan.','saved':'Data pegawai berhasil diperbarui.','added':'Pegawai berhasil ditambahkan.'},'ms':{'bad':'Semak nama, jabatan, bahagian, telefon dan e-mel pekerja.','password':'Kata laluan sementara mestilah sekurang-kurangnya 8 aksara.','email':'E-mel ini telah digunakan.','saved':'Maklumat pekerja berjaya dikemas kini.','added':'Pekerja berjaya ditambah.'}};return messages.get(lang,messages['en'])[key]

    def valid_employee_fields(name,department,job_title,mobile,email):
        digits=''.join(ch for ch in mobile if ch.isdigit());return 2<=len(name)<=120 and 2<=len(department)<=100 and 2<=len(job_title)<=120 and 8<=len(digits)<=15 and len(email)<=254 and '@' in email and '.' in email.rsplit('@',1)[-1]
    ...
    def edit_employee_extended(uid):
        ...
        if not valid_employee_fields(name,department,job_title,mobile,email):c.close();flash(employee_error('bad'));return redirect(url_for('admin_employees'))
        if c.execute('SELECT 1 FROM users WHERE email=? AND id<>?',(email,uid)).fetchone():c.close();flash(employee_error('email'));return redirect(url_for('admin_employees'))
    ...
    def offer_error(u,key):
        lang=(u['language'] if u else 'ar') or 'ar';messages={'ar':{'bad':'تعذر حفظ العرض. تحقق من البيانات المطلوبة.','dates':'تاريخ نهاية العرض يجب ألا يسبق تاريخ البداية.','hotel':'الفندق المحدد غير موجود أو غير نشط.','selected':'اختر وكالة واحدة على الأقل للعرض المخصص.','image':'رابط صورة العرض غير صالح.'},'en':{'bad':'The offer could not be saved. Check the required fields.','dates':'Offer end date cannot be before the start date.','hotel':'The selected hotel does not exist or is inactive.','selected':'Select at least one agency for a targeted offer.','image':'The offer image URL is invalid.'},'id':{'bad':'Penawaran tidak dapat disimpan. Periksa data wajib.','dates':'Tanggal akhir penawaran tidak boleh sebelum tanggal mulai.','hotel':'Hotel yang dipilih tidak ditemukan atau tidak aktif.','selected':'Pilih minimal satu agen untuk penawaran khusus.','image':'URL gambar penawaran tidak valid.'},'ms':{'bad':'Tawaran tidak dapat disimpan. Semak maklumat wajib.','dates':'Tarikh tamat tawaran tidak boleh sebelum tarikh mula.','hotel':'Hotel yang dipilih tidak wujud atau tidak aktif.','selected':'Pilih sekurang-kurangnya satu agensi untuk tawaran khusus.','image':'URL imej tawaran tidak sah.'}};return messages.get(lang,messages['en'])[key]
    ...
    def validate_offer(c,u):
        ...
        if hotel_raw:
            ...
            if not c.execute('SELECT id FROM hotels WHERE id=? AND active=1',(hotel_id,)).fetchone():return None,offer_error(u,'hotel')
        ...
        if selected:
            placeholders=','.join('?' for _ in selected);valid_ids={r['id'] for r in c.execute(f"SELECT a.id FROM agencies a JOIN users u ON u.id=a.user_id WHERE a.id IN ({placeholders}) AND u.active=1",selected).fetchall()};selected=[sid for sid in selected if sid in valid_ids]
    ...
    def send_offer_notifications(c,oid,title,audience,selected,language_mode,manual_language):
        rows=c.execute("SELECT a.id agency_id,u.id user_id,u.language,a.country FROM agencies a JOIN users u ON u.id=a.user_id WHERE u.active=1").fetchall();selected=set(selected);labels={'ar':('عرض جديد','تمت إضافة عرض جديد: '),'en':('New offer','A new offer is available: '),'id':('Penawaran baru','Penawaran baru tersedia: '),'ms':('Tawaran baharu','Tawaran baharu tersedia: ')}
        for row in rows:
            ...
            lang=manual_language if language_mode=='manual' else (row['language'] or 'en');head,prefix=labels.get(lang,labels['en']);c.execute("INSERT INTO notifications(user_id,type,ref_id,title,body,link,created_at) VALUES(?,?,?,?,?,?,?)",(row['user_id'],'offer',oid,head,prefix+title,f'/offer/{oid}',datetime.utcnow().isoformat()))

    def offer_allowed(c,offer,u):
        audience=offer['audience'] or 'all'
        if audience=='all':return True
        if not u or u['role']!='agency':return False
        agency=c.execute("SELECT id,country FROM agencies WHERE user_id=?",(u['id'],)).fetchone()
        if not agency:return False
        country=(agency['country'] or '').lower()
        if audience=='indonesia':return 'indonesia' in country
        if audience=='malaysia':return 'malaysia' in country
        if audience=='selected':return bool(c.execute("SELECT 1 FROM offer_targets WHERE offer_id=? AND agency_id=?",(offer['id'],agency['id'])).fetchone())
        return False

    def home_extended():
        ensure_schema();c=db();u=current_user();today=datetime.utcnow().date().isoformat();hotels=c.execute("SELECT h.*,(SELECT image_url FROM hotel_images i WHERE i.hotel_id=h.id ORDER BY i.sort_order,i.id LIMIT 1) cover_image FROM hotels h WHERE h.active=1 ORDER BY h.sort_order,h.id").fetchall();base="""SELECT o.*,h.name_ar,h.name_en FROM offers o LEFT JOIN hotels h ON h.id=o.hotel_id WHERE o.active=1 AND (o.start_date='' OR o.start_date IS NULL OR o.start_date<=?) AND (o.end_date='' OR o.end_date IS NULL OR o.end_date>=?)""";params=[today,today]
        if u and u['role']=='agency':
            ...
            if agency:
                ...
                placeholders=','.join('?' for _ in allowed);base+=f" AND (o.audience IN ({placeholders}) OR (o.audience='selected' AND EXISTS (SELECT 1 FROM offer_targets ot WHERE ot.offer_id=o.id AND ot.agency_id=?)))";params.extend(allowed);params.append(agency['id'])
    ...
    def offer_detail_extended(oid):
        ensure_schema();c=db();today=datetime.utcnow().date().isoformat();offer=c.execute("""SELECT o.*,h.name_ar,h.name_en,h.city,(SELECT image_url FROM hotel_images i WHERE i.hotel_id=o.hotel_id ORDER BY i.sort_order,i.id LIMIT 1) cover_image FROM offers o LEFT JOIN hotels h ON h.id=o.hotel_id WHERE o.id=? AND o.active=1 AND (o.start_date='' OR o.start_date IS NULL OR o.start_date<=?) AND (o.end_date='' OR o.end_date IS NULL OR o.end_date>=?)""",(oid,today,today)).fetchone();u=current_user()
        if not offer or not offer_allowed(c,offer,u):c.close();return 'offer not found',404
        c.close();return render_template('offer.html',offer=offer,user=u)
